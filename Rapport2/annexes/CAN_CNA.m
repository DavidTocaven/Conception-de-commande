...
figure(1)
    ...
    %p(2).LineWidth = 2;
    %p(1).LineWidth = 2,
        title('Convertion Analogique Numerique')
...
title('Convertion Numerique Analogique')
...
p=plot(0,0,'>b',5,2047,'<b',x,y,'b',x,yGain,'r',x,yOffs,'--g',x,yInt,'-*k')
    ...
    p(2).LineWidth=1.5;
    p(3).LineWidth=2;
    title('CAN : differentes erreurs de conversions')
    legend('min','max','theorique','Erreur gain','Erreur d''offset','non-linearite integrale','Location','SouthEast');
    axis([0 5 0 2050])        
    xlabel('Tension (Volt)');
