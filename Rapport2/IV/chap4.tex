...
\section{Protocoles MIL/SIL}
	\subsection{MIL}
	Pour faire une validation MIL, nous allons utiliser le modèle non linéaire pour simuler la correction de celui ci avec notre correcteur. Toutes cette opération sera effectué sous MATLAB. Pour valider la commande du correcteur, celle ci devra être ré-étudié pour l'améliorer. Nous devons alors fixer une marge d'erreur par rapport au cahier des charges défini en \ref{chap:commande} : 
