...
Notre modèle présente 4 dynamiques. Elles sont présentent sur :
\begin{itemize}
\item $i_1(t)$ à cause de la présence de l'inductance de charge $L$. ($u(t) = L \frac{di(t)}{dt}$ donc il faut la valeur de charge initiale.)
\item $i_2(t)$ pour la même raison que $i_1(t)$.
\item $\omega$ à cause de l
\item $\theta$ 
\end{itemize}
\hspace{5mm} \textbullet \hspace{5mm} Variable d'état :
\begin{equation}
\begin{pmatrix}
\theta_s(t)\\
\omega(t)\\
